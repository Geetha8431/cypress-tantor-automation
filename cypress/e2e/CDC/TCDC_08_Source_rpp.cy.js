...
describe('Source Connector Restart, Pause, Resume', () => {
...
cy.xpath("(//button[contains(., ' Restart')])[1]").should("be.visible").click();
cy.wait(2000);
cy.screenshot('src-connector-restart');


// ---------- Pause Source Connector ----------------
cy.xpath("(//button[contains(., 'Pause')])[1]").should('be.visible').click();
cy.wait(2000);
cy.screenshot('src-connector-pause');
...
cy.wait(2000);
cy.screenshot('src-connector-resume');
